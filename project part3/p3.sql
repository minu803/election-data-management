...
-- Table 5.1: residential_address_info_test
DROP TABLE IF EXISTS residential_address_info_test;
CREATE TABLE IF NOT EXISTS residential_address_info_test(
	voter_reg_num BIGINT(20),
    house_num VARCHAR(45),
    street_dir VARCHAR(45),
    street_type_cd VARCHAR(45),
    ...
    zip_code CHAR(5),
    pct_portion CHAR(200),
    PRIMARY KEY (zip_code, pct_portion,voter_reg_num),
    FOREIGN KEY (zip_code) REFERENCES zip_city(zip_code),
    FOREIGN KEY (voter_reg_num) REFERENCES person(voter_reg_num)
);

INSERT IGNORE INTO residential_address_info_test
SELECT voter_reg_num, house_num, street_dir,street_type_cd, street_sufx_cd,
	unit_num, zip_code, pct_portion
FROM voter;
...
SELECT 	voter_reg_num, E1 AS election_ID, 
		E1_VotingMethod AS voting_method, 
        E1_PartyCd AS party_cd
FROM voter
WHERE E1_date IS NOT NULL
UNION ALL
SELECT voter_reg_num, E2 AS election_ID, E2_VotingMethod AS voting_method, E2_PartyCd AS party_cd
from voter
WHERE E2_date IS NOT NULL
UNION ALL
SELECT voter_reg_num, E3 AS election_ID,  E3_VotingMethod AS voting_method, E3_PartyCd AS party_cd
from voter
WHERE E3_date IS NOT NULL
UNION ALL
SELECT voter_reg_num, E4 AS election_ID,  E4_VotingMethod AS voting_method, E4_PartyCd AS party_cd
from voter
WHERE E4_date IS NOT NULL
UNION ALL
SELECT voter_reg_num,E5 AS election_ID,  E5_VotingMethod AS voting_method, E5_PartyCd AS party_cd
from voter
...
select * from voter WHERE res_city_desc IS NOT NULL;
#######################################################################################################################
